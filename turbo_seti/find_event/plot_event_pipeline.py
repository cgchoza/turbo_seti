...
def plot_event_pipeline(event_csv_string, fils_list_string, event_id_string=None, user_validation=False,
                        offset=0, filter_spec=None, sortby_tstart=True, plot_dir=None, encoding='utf-8', single=False):
    r"""
    ...
    single: bool
        If True, only one event with the given event_id_string is plotted. This treats 
    ...
    >>> import plot_event_pipeline
    ... plot_event_pipeline.plot_event_pipeline(event_csv_string, single=True, fils_list_string)
    ...
    fil_file_list = []
    for file in pandas.read_csv(fils_list_string, encoding=encoding, header=None, chunksize=1):
    ...
    num_events = str(len(candidate_event_dataframe.Source))

    if not single and event_id_string is None:
        print(f"Plotting some events for: {on_source_name}")
        print(
            f"There are {num_events} total events in the csv file {event_csv_string}")
        print(f"therefore, you are about to make {num_events} .png files.")
    elif single:
        print(f"Plotting the first event for: {on_source_name}")
        print(
            f"There are {num_events} total events in the csv file {event_csv_string}")
        print(f"You are about to make {1} .png file.")
    elif event_id_string is not None and not single:
        num_for_id = len(
            candidate_event_dataframe.loc[candidate_event_dataframe["Hit_ID"] == event_id_string])
        print(f"Plotting some events for: {on_source_name}")
        print(
            f"There are {num_for_id} total events with the ID {event_id_string} in the csv file {event_csv_string}")
        print(f"therefore, you are about to make {num_for_id} .png files.")
    elif single and event_id_string is not None:
        num_for_id = len(
            candidate_event_dataframe.loc[candidate_event_dataframe["Hit_ID"] == event_id_string])
        print(
            f"There are {num_for_id} total events with the ID {event_id_string} in the csv file {event_csv_string}")
        print(f"Plotting an event for: {on_source_name}")
        print(f"You are about to make {1} .png file.")
